...
public class ChestSpawner : MonoBehaviour
{
    ...
    #endregion
    [SerializeField] private List<SpawnableObjectByLevel<WeaponsDetailsSO>> weaponSpawnByLevelList;
    ...
    private void OnEnable()
    {
        ...
        StaticEventHandler.OnRoomEnemiesDefeated += StaticEventHandle_OnRoomEnemiesDefeated;
    }
    ...
    private void StaticEventHandle_OnRoomEnemiesDefeated(RoomEnemiesDefeatedArgs roomEnemiesDefeatedArgs)
    {
        if (chestRoom == null)
        {
            chestRoom = GetComponentInParent<InstantiatedRoom>().room;
    ...
    private WeaponsDetailsSO GetWeaponDetailsToSpawn(int weaponNumber)
    {
        if (weaponNumber == 0) return null;

        RandomSpawnableObject<WeaponsDetailsSO> weaponRandom = new RandomSpawnableObject<WeaponsDetailsSO>(weaponSpawnByLevelList);
    ...
    private void OnValidate()
    {
        ...
        {
            foreach (SpawnableObjectByLevel<WeaponsDetailsSO> weaponDetailsByLevel in weaponSpawnByLevelList)
            {
                HelperUtilities.ValidateCheckNullValue(this, nameof(weaponDetailsByLevel.dungeonLevel), weaponDetailsByLevel.dungeonLevel);

                foreach (SpawnableObjectRatio<WeaponsDetailsSO> weaponRatio in weaponDetailsByLevel.spawnableObjectRatioList)
                {
